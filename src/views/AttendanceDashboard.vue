<template>
    <div class="attendance-dashboard">
      ...
      <div class="filters">
        ...
        <select v-model="selectedDay" @change="loadData">
        ...
        <div v-if="activeTab === 'student'" class="student-filter">
          ...
          <select v-if="selectedStudent" v-model="studentQueryType" @change="loadData">
            <option value="day">Single Day</option>
            <option value="month">Single Month</option>
            <option value="span">Month Span</option>
          </select>
          <div v-if="studentQueryType === 'span'" class="month-range">
            <select v-model="startMonth" @change="loadData">
              <option v-for="(month, index) in months" :key="index" :value="index + 1">{{ month }}</option>
            </select>
            <select v-model="endMonth" @change="loadData">
              <option v-for="(month, index) in months" :key="index" :value="index + 1">{{ month }}</option>
            </select>
          </div>
        </div>
      ...
      <div class="tabs">
        <button :class="{ active: activeTab === 'absent' }" @click="activeTab = 'absent'">Absent Students</button>
        <button :class="{ active: activeTab === 'monthly' }" @click="activeTab = 'monthly'">Monthly Report</button>
        <button :class="{ active: activeTab === 'quarterly' }" @click="activeTab = 'quarterly'">Quarterly Report</button>
        <button :class="{ active: activeTab === 'low' }" @click="activeTab = 'low'">Low Attendance</button>
        <button :class="{ active: activeTab === 'student' }" @click="activeTab = 'student'">Student Attendance</button>
      </div>
      <div v-if="activeTab === 'absent'" class="table-container">
        ...
        <button @click="exportToCSV('absent')">Export to CSV</button>
      </div>
      <div v-if="activeTab === 'monthly' || activeTab === 'quarterly'" class="table-container">
        ...
        <button @click="exportToCSV(activeTab)">Export to CSV</button>
      </div>
      <div v-if="activeTab === 'low'" class="table-container">
        ...
        <button @click="exportToCSV('low')">Export to CSV</button>
      </div>
      <div v-if="activeTab === 'student' && selectedStudent" class="table-container">
        ...
        <table v-if="studentQueryType === 'day' && studentAttendance.day">
          <thead>
            <tr>
              <th>Date</th>
              <th>Class</th>
        ...
        <button v-if="studentAttendance.records.length" @click="exportStudentAttendanceToCSV">Export to CSV</button>
      </div>
      <Snackbar
        ...
        if (this.selectedClass) {
          ...
          const studentDocs = await Promise.all(
            studentIds.map(async studentId => {
              ...
              return docs.flat().filter(doc => doc);
            })
          );
          records = studentDocs.flat();
        } else {
